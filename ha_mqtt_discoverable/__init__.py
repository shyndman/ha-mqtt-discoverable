...
type TopicGenerator[E] = Callable[[E], str]
...
class Discoverable(Generic[EntityType]):
    ...
    def __init__(
        ...
        make_state_topic: Optional[TopicGenerator[Settings[EntityType]]] = None,
        ...
        self.state_topic = (
            ...
            if make_state_topic is None
            else make_state_topic(settings)
        )

        # Full topic where we publish our own attributes as JSON messages
        # Prepend the `state_prefix`, default: `hmd`
        # e.g. hmd/binary_sensor/mydevice/mysensor
        self.attributes_topic = f"{self.state_topic}/attributes"

        logger.info(f"config_topic: {self.config_topic}")
        logger.info(f"state_topic: {self.state_topic}")
        logger.info(f"attributes_topic: {self.attributes_topic}")
...
wrote_configuration: {self.wrote_configuration}
    ...
    def _connect_client(self) -> None:
        ...
        result = self.mqtt_client.connect(
            self._settings.mqtt.host, self._settings.mqtt.port or 1883
        )
