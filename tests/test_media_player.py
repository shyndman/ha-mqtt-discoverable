...
def test_partial_media_player_creation(partial_media_player):
    ...
    assert set(partial_media_player._callbacks.keys()) == {
        "play",
        "pause",
        "volume_set",
        "shuffle_set",
...
def test_all_players_have_state_topics():
    ...
    for player_name, callbacks in players:
        ...
        for topic_key in expected_state_topics:
            assert topic_key in player._topics, (
...
def test_command_routing_multiple_commands():
    ...
    publish.single(
        player._topics[MediaPlayerTopics.PAUSE], "PAUSE", hostname="localhost"
    )
    publish.single(
        player._topics[MediaPlayerTopics.VOLUME_SET], "0.5", hostname="localhost"
    )
...
def test_rapid_command_sequence():
    ...
    publish.single(
        player._topics[MediaPlayerTopics.VOLUME_SET], "0.7", hostname="localhost"
    )
...
def test_parse_command_payload_string_commands():
    ...
    assert (
        player._parse_command_payload(MediaPlayerTopics.SELECT_SOURCE, "CD Player")
        == "CD Player"
    )
    assert (
        player._parse_command_payload(MediaPlayerTopics.SELECT_SOURCE, "Bluetooth")
        == "Bluetooth"
    )
    assert (
        player._parse_command_payload(MediaPlayerTopics.SELECT_SOUND_MODE, "Movie")
        == "Movie"
    )
...
def test_parse_command_payload_invalid_numeric():
    ...
    assert (
        player._parse_command_payload(MediaPlayerTopics.VOLUME_SET, "invalid") is None
    )
...
def test_parse_command_payload_edge_cases():
    ...
    assert (
        player._parse_command_payload(MediaPlayerTopics.VOLUME_SET, "") is None
    )  # Invalid float

    # Whitespace
    assert (
        player._parse_command_payload(MediaPlayerTopics.SELECT_SOURCE, "  Spotify  ")
        == "  Spotify  "
    )

    # Boolean edge cases
    assert (
        player._parse_command_payload(MediaPlayerTopics.SHUFFLE_SET, "True") is False
    )  # Only "ON" is True
    assert (
        player._parse_command_payload(MediaPlayerTopics.SHUFFLE_SET, "1") is False
    )  # Only "ON" is True
